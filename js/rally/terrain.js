...
function groundLabel(scene, text, x, z, width) {
  ...
  ctx.font = "600 118px 'Fira Code', monospace";
  ctx.textAlign = "center";
  ctx.textBaseline = "middle";
  ctx.lineWidth = 14;
  ctx.strokeStyle = "#1a1f2488";
  ctx.strokeText(text, 512, 100, 980);
  ctx.fillStyle = "#e7e2d0";
  ctx.fillText(text, 512, 100, 980);
...
function drawContours(context) {
  const step = 6,
    columns = Math.ceil(WORLD.width / step),
    rows = Math.ceil(WORLD.depth / step);
  const heights = new Float32Array((columns + 1) * (rows + 1));
  for (let row = 0; row <= rows; row++)
    for (let col = 0; col <= columns; col++) {
      heights[row * (columns + 1) + col] = terrainHeight(
        col * step - WORLD.width / 2,
        row * step - WORLD.depth / 2,
      );
    }
  context.beginPath();
  for (let row = 0; row < rows; row++)
    for (let col = 0; col < columns; col++) {
      const x = col * step - WORLD.width / 2,
        z = row * step - WORLD.depth / 2;
      const i = row * (columns + 1) + col;
      const corners = [
        [x, z, heights[i]],
        [x + step, z, heights[i + 1]],
        [x + step, z + step, heights[i + columns + 2]],
        [x, z + step, heights[i + columns + 1]],
      ];
      const min = Math.min(...corners.map((point) => point[2])),
        max = Math.max(...corners.map((point) => point[2]));
      for (let level = Math.max(16, Math.ceil(min / 16) * 16); level < max; level += 16) {
        ...
        for (let p = 0; p < intersections.length - 1; p += 2) {
          context.moveTo(...intersections[p]);
          context.lineTo(...intersections[p + 1]);
        }
      }
    }
...
export function makeGround(scene, routes, landmarks) {
  const canvas = document.createElement("canvas");
  canvas.width = 3600;
  canvas.height = Math.round((3600 * WORLD.depth) / WORLD.width);
  ...
  for (let z = -WORLD.depth / 2; z < WORLD.depth / 2; z += 6)
    for (let x = -WORLD.width / 2; x < WORLD.width / 2; x += 6) {
      const y = terrainHeight(x + 3, z + 3),
        slope = terrainGradient(x + 3, z + 3);
  ...
  context.beginPath();
  for (let x = -WORLD.width / 2; x <= WORLD.width / 2; x += 6) {
    context.moveTo(x, -WORLD.depth / 2);
    context.lineTo(x, WORLD.depth / 2);
  }
  for (let z = -WORLD.depth / 2; z <= WORLD.depth / 2; z += 6) {
    context.moveTo(-WORLD.width / 2, z);
    context.lineTo(WORLD.width / 2, z);
  }
  context.stroke();
  ...
  context.beginPath();
  for (let x = -390; x < 400; x += 30)
    for (let z = -450; z < 460; z += 30) {
      context.moveTo(x - 0.7, z);
      context.lineTo(x + 0.7, z);
      context.moveTo(x, z - 0.7);
      context.lineTo(x, z + 0.7);
    }
  context.stroke();
  ...
  for (const camp of CAMPS) {
    context.save();
    context.translate(camp.x, camp.z);
    context.rotate(camp.heading);
  ...
  if (COTTAGE) {
    context.beginPath();
    context.moveTo(...COTTAGE.path[0]);
    context.quadraticCurveTo(...COTTAGE.path[1], ...COTTAGE.path[2]);
    ...
    context.save();
    context.translate(COTTAGE.parking.x, COTTAGE.parking.z);
    context.rotate(COTTAGE.parking.heading);
    context.fillStyle = "#4a463c80";
    context.beginPath();
    context.ellipse(0, 0, 4.6, 3.4, 0, 0, Math.PI * 2);
    context.fill();
    context.fillStyle = "#6b615266";
    context.beginPath();
    context.ellipse(0, 0, 3.6, 2.5, 0, 0, Math.PI * 2);
    context.fill();
    context.restore();
  }
  for (const route of routes) {
    ...
    for (const [color, width] of [
      ["#3e423d", route.width + 3],
      ["#5e5649", route.width],
      [route.tint, route.width - 1.1],
      ["#675c4b", route.width * 0.4],
    ]) {
      context.strokeStyle = color;
      context.lineWidth = width;
      context.stroke();
    }
    context.strokeStyle = "#242a2440";
    ...
    for (let i = 13; i < route.samples.length - 1; i += 23 + Math.floor(random() * 12)) {
      const [x, z] = route.samples[i],
        next = route.samples[i + 1];
  ...
  for (let i = 0; i < 70000; i++) {
    ...
    context.fillRect(
      random() * WORLD.width - WORLD.width / 2,
      random() * WORLD.depth - WORLD.depth / 2,
      random() * 0.3 + 0.05,
      random() * 0.2 + 0.05,
    );
  }
  context.save();
  context.translate(SPAWN.x, SPAWN.z);
  context.rotate(SPAWN.heading);
  context.fillStyle = "#615a4c";
  context.beginPath();
  context.ellipse(0, 0, 13, 12, 0, 0, Math.PI * 2);
  context.fill();
  ...
  context.strokeStyle = "#20262660";
  context.lineWidth = 0.25;
  for (const radius of [4.1, 5.3]) {
    context.beginPath();
    context.ellipse(0, 0, radius, radius * 0.8, -0.4, 0, 5.4);
    context.stroke();
  }
  context.restore();

  context.save();
  context.translate(FINISH_LINE.x, FINISH_LINE.z);
  context.rotate(FINISH_LINE.heading);
  context.fillStyle = "#615a4c";
  context.beginPath();
  ...
  const tiles = 16,
    tileWidth = WORLD.width / tiles,
    tileDepth = WORLD.depth / tiles;
  const mountain = new THREE.Group();
  mountain.name = "terrain";
  for (let row = 0; row < tiles; row++)
    for (let col = 0; col < tiles; col++) {
      const x = (col + 0.5) * tileWidth - WORLD.width / 2,
        z = (row + 0.5) * tileDepth - WORLD.depth / 2;
      const geometry = new THREE.PlaneGeometry(tileWidth, tileDepth, 528 / tiles, 592 / tiles);
      geometry.rotateX(-Math.PI / 2);
      const positions = geometry.attributes.position,
        uv = geometry.attributes.uv;
      for (let i = 0; i < positions.count; i++) {
        const wx = positions.getX(i) + x,
          wz = positions.getZ(i) + z;
        positions.setY(i, terrainHeight(wx, wz));
        // Every tile samples one world-sized texture, so roads and contours join across tile edges.
  ...
  const valley = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({ color: "#252c32", roughness: 1 }),
  );
