...
export async function start() {
  ...
  if (!response.ok)
    throw new Error(`Could not load resume (${response.status}).`);
  const resume = await response.json();
  const landmarks = createLandmarks(resume);
  if (!landmarks.length)
    throw new Error("No places to discover in this resume.");
  let storage;
  try {
  ...
  if (stops.checkpoint) {
    ...
    $("intro-description").textContent =
      `Continue from ${stops.checkpoint.name}. Your checkpoint and field notes are saved.`;
    $("start-button").firstChild.textContent = "Continue driving ";
  }
  document.body.classList.toggle(
    "is-first-visit",
    !stops.checkpoint && !visits.size,
  );
  let started = false,
    blurred = false,
    stopped = false;
  ...
  const updateSecrets = () => {
    $("secret-count").textContent =
      `SECRETS ${pad(rest.found.size)} / ${pad(rest.spots.length)}${rest.found.size === rest.spots.length ? " · you found them all" : " · stop by anything odd off the road"}`;
  };
  updateSecrets();
  let accumulator = 0,
    uiElapsed = 1,
  ...
  let shownCheckpoint = stops.checkpoint,
    orbitPaused = reducedMotion.matches;
  const labels = [],
    mapDots = [],
    slots = [],
    destinations = [];
  ...
  for (const sight of landmarks) {
    ...
    number.textContent = pad(sight.index + 1);
    name.append(
      number,
      document.createTextNode(
        sight.name === "Supplier Success Accelerator"
          ? "Supplier Success"
          : sight.name,
      ),
    );
    const subtitle = document.createElement("small");
    subtitle.textContent = `${sight.label} / ${Math.round(sight.elevation)} M`;
    label.append(name, subtitle);
    $("world-labels").append(label);
    labels.push(label);

    const dot = svgElement("g", {
      transform: `translate(${sight.x} ${sight.z})`,
      class: "map-dot",
      role: "button",
      tabindex: "0",
      "aria-label": `Go to ${sight.name}`,
    });
    ...
    numeral.textContent = sight.index + 1;
  ...
  const crossingLabels = CROSSINGS.map((crossing) => {
    ...
    element.className = "world-label crossing-label";
    element.style.setProperty(
      "--sight-color",
      crossing.type === "jump" ? "#e5c07b" : "#76c9d4",
    );
    const title = document.createElement("span");
    title.className = "world-label-name";
    title.textContent = `${crossing.type === "jump" ? "↗" : "≈"} ${crossing.label}`;
    const caption = document.createElement("small");
    caption.textContent =
      crossing.type === "jump"
        ? "KEEP YOUR SPEED / CLEAR THE RIVER"
        : crossing.style === "timber"
          ? "NO RAILS / MIND THE EDGE"
    ...
    return {
      element,
      ...crossingPoint(crossing, along),
      y: crossingDeckHeight(crossing, along) + 7,
    };
  });
  for (const [line, title, caption] of [
    [START_LINE, "START / THE DESCENT", "CROSS THE LINE TO START THE CLOCK"],
    [FINISH_LINE, "FINISH", "ALL NINE CHECKPOINTS / ONE DESCENT"],
  ]) {
    const element = document.createElement("div");
    element.className = "world-label crossing-label";
  ...
  function updateProgress() {
    ...
    remainder.textContent = `/ ${pad(landmarks.length)}`;
    $("visit-count").replaceChildren(
      document.createTextNode(`${pad(visits.size)} `),
      remainder,
    );
    ...
    $("visit-caption").textContent =
      visits.size === landmarks.length
        ? "every story found"
        : "places discovered";
    if (visits.size === landmarks.length)
      $("next-stop").textContent = "Every chapter. A whole mountain of work.";
  }
  updateProgress();
  updateCheckpoint();
  updateTiming();
  ...
  function updateTiming() {
    const running = timing.status === "running",
      finished = timing.status === "finished";
    $("race-panel").dataset.status = timing.status;
    $("race-status").textContent = finished
      ? "FINISH / ALL CHECKPOINTS"
      : running
        ? isPaused()
          ? "DESCENT / PAUSED"
          : "DESCENT / ON THE CLOCK"
        : timing.status === "ready"
          ? "READY / CROSS THE START LINE"
          : "FREE ROAM / START AT SUMMIT";
    $("race-time").textContent = formatTime(
      timing.status === "idle" ? null : timing.elapsed,
    );
    const index = timing.times.length - 1;
    $("race-split-label").textContent =
      index >= 0
        ? `${finished ? "FINISH" : `CP ${pad(index + 1)}`} · ${formatTime(timing.times[index])}`
        : "PREVIOUS DESCENT";
    const delta = index >= 0 ? timing.delta(index) : null;
    $("race-delta").textContent =
      index >= 0
        ? formatDelta(delta)
        : timing.previous
          ? formatTime(timing.previous.at(-1))
          : "—";
    $("race-delta").dataset.pace =
      delta === null ? "" : delta < 0 ? "ahead" : "behind";
    $("race-next").textContent = timing.next
      ? timing.next.id === "finish"
        ? "Next: the finish arch ↓"
        : `Next: ${pad(timing.next.index + 1)} / ${timing.next.name}`
      : finished
        ? "Descent complete. Another run?"
        : "Start in the circle before Rowte.io.";
    landmarks.forEach((sight, i) => {
      const current = timing.times[i],
        previous = timing.reference?.[i];
      destinations[i].querySelector(".travel-split").textContent =
  ...
  function updateCheckpoint() {
    ...
    $("checkpoint-button").setAttribute(
      "aria-label",
      `Restart from ${name} (R)`,
    );
    ...
    landmarks.forEach((sight, index) => {
      ...
      destinations[index].classList.toggle(
        "checkpoint",
        stops.checkpoint === sight,
      );
    });
  }
  ...
  function isPaused() {
    // Reading freezes driving and the race clock, but its camera may keep animating.
    return isSuspended() || Boolean(stops.viewing);
  }

  function syncRendering(invalidate = false) {
    const visible =
      !blurred && !document.hidden && !portrait.matches && !stopped;
    let fps = 0;
    if (!isSuspended()) {
      if (!stops.viewing) fps = GRAPHICS_PROFILES[quality].fps;
      // Let the camera ease into its final pose before freezing a still project view.
      else if (
        (!orbitPaused && !reducedMotion.matches) ||
        performance.now() < settleUntil
      )
        fps = Math.min(30, GRAPHICS_PROFILES[quality].fps);
    }
  ...
  function focusGame() {
    if (started && !isSuspended())
      $(stops.viewing ? "project-title" : "rally-scene").focus({
        preventScroll: true,
      });
  }

  function openHelp() {
    if (stopped || portrait.matches || dialogs.some((dialog) => dialog.open))
      return;
  ...
  function travelTo(sight) {
    ...
    resetCar(car, pose);
    if (sight) stops.activate(sight);
    else stops.resetToSummit();
    stops.clearApproach();
    stops.update(car, 0);
    shownCheckpoint = stops.checkpoint;
    updateCheckpoint();
    lastSafePose = { ...pose };
    timing.travel(car);
    recovery.reset(car);
    graphics.crashEffect.clear();
    updateTiming();
    graphics.splashEffect.clear();
    graphics.fireworks.clear();
    graphics.snapCamera(car);
    beginDriving();
    dialogs.forEach((dialog) => {
      if (dialog.open) dialog.close();
    ...
    announce(
      !sight
        ? "Summit start. Cross the chequered line to start the clock."
        : `${sight.name} · Free roam. New run returns to the start circle.`,
    );
  }
  ...
  function showProject() {
    ...
    $("project-eyebrow").textContent =
      `${pad(sight.index + 1)} / ${sight.label} / ${Math.round(sight.elevation)} M`;
    $("project-title").textContent = sight.name;
    $("project-meta").textContent =
      `${sight.company} · ${sight.role} · ${sight.period}`;
    ...
    for (const subsection of sight.subsections) {
      ...
      heading.textContent = subsection.title || "What I built";
    ...
    let url;
    try {
      url = new URL(sight.url);
    } catch {
      /* Some projects have no public website. */
    }
  ...
  function updateOrbitButton() {
    ...
    $("orbit-button").textContent = orbitPaused
      ? "Resume camera ▶"
      : "Pause camera Ⅱ";
  ...
  function setNearby(sight) {
    ...
    $("stop-hint").textContent = stops.ready
      ? "Parked. Take a closer look."
      : "Brake to a stop to explore.";
  ...
  function discover() {
    if (stops.reached && !visits.has(stops.reached.id)) {
      ...
      updateProgress();
      if (visits.size === landmarks.length)
        announce("Every story found. What a ride. Thanks for coming along!");
    }
    setNearby(stops.nearby);
    const progress = roadAt(car.x, car.z)?.progress ?? 0;
    const next = landmarks.find(
      (sight) => !visits.has(sight.id) && sight.courseDistance >= progress - 1,
    );
    if (next && visits.size < landmarks.length)
      $("next-stop").textContent =
        `Next lookout: ${next.name === "Supplier Success Accelerator" ? "Supplier Success" : next.name}`;
    else if (visits.size < landmarks.length)
      $("next-stop").textContent =
        `${landmarks.length - visits.size} stories left — choose one in Projects ↗`;
    if (timing.status === "finished")
      $("next-stop").textContent =
        "Descent complete. Every checkpoint cleared.";
    else if (timing.next)
      $("next-stop").textContent =
        timing.next.id === "finish"
          ? "All checkpoints cleared. Finish under the arch ↓"
          : `Next checkpoint: ${pad(timing.next.index + 1)} / ${timing.next.name}`;
    else if (visits.size === landmarks.length)
      $("next-stop").textContent = "Every chapter. A whole mountain of work.";
  }
  ...
  function restartCheckpoint() {
    ...
    stops.clearApproach();
    stops.update(car, 0);
    lastSafePose = stops.restartPose;
    recovery.reset(car);
    graphics.crashEffect.clear();
    graphics.splashEffect.clear();
    graphics.fireworks.clear();
  ...
  $("start-button").addEventListener("click", () => {
    ...
    if (window.matchMedia("(pointer: coarse)").matches)
      announce(
        "Left half: steer left and right buttons. Right half: gas, brake and handbrake.",
      );
  });
  for (const id of ["help-button", "intro-help-button"])
  ...
  for (const id of ["travel-button", "map-button", "intro-travel-button"])
    $(id).addEventListener("click", openTravel);
  ...
  for (const id of ["close-project", "resume-driving"])
    $(id).addEventListener("click", () => closeProject());
  ...
  $("expand-project").addEventListener("click", () => {
    ...
    $("expand-project").textContent = expanded
      ? "Compact view ↔"
      : "Wider view ↔";
    settleUntil = performance.now() + 2500;
    syncRendering(true);
  ...
  panel.addEventListener("keydown", (event) => {
    if (
      ...
      event.metaKey ||
      event.altKey ||
      !["Escape", "KeyE", "KeyR", "KeyM"].includes(event.code)
    )
      return;
    event.preventDefault();
    event.stopPropagation();
    input.onAction(event.code);
  });
  ...
  $("sound-button").addEventListener("click", async () => {
    try {
      ...
      announce(
        "Sound is unavailable in this browser. The road is still yours.",
      );
    }
    focusGame();
  });
  ...
  $("rally-scene").addEventListener("webglcontextlost", (event) => {
    ...
    $("error-message").textContent =
      "The graphics connection was interrupted. Reload to get back on the road; your discoveries are saved.";
  ...
  function frame(time, dt) {
    ...
    if (!paused) {
      ...
      const environment = {
        rails: RAILS,
        snowLine: SNOW_LINE,
        heightAt: driveHeightAt,
        supportAt: driveHeightAt,
        waterAt: riverAt,
      };
      // Spend elapsed driving time in fixed steps so graphics quality does not change handling.
      // Collision candidates are refreshed per step because one rendered frame can cross cells.
      while (accumulator >= FIXED_STEP) {
        ...
        if (event === "water-entry") {
          graphics.splashEffect.splash(
            car,
            riverAt(car.x, car.z)?.y ?? car.y,
          ...
          announce(
            event === "splash"
              ? "Splashed down. Back on the bank — keep some speed for the jump!"
              : `Back at ${stops.checkpoint?.name || "the summit"}.${timing.status === "running" ? " The clock keeps running." : " Ready to drive."}`,
          );
        }
      }
    } else accumulator = 0;
    if (!paused) {
      ...
      if (shownCheckpoint !== stops.checkpoint) {
        ...
        updateCheckpoint();
        announce(
          `${stops.checkpoint.name} · Checkpoint saved. R restarts here.`,
        );
      }
      ...
      if (split) {
        discover();
        if (split.finished)
          graphics.fireworks.launch(FINISH_LINE, reducedMotion.matches);
        announce(
          `${split.finished ? "FINISH" : `CP ${pad(split.index + 1)}`} · ${formatTime(split.time)} · ${formatDelta(split.delta)}${split.delta === null ? "" : " vs previous descent"}`,
        );
      }
    }
    const arrival = rest.update(car, dt, !paused && car.grounded);
    if (arrival.arrived) audio.cue(rest.visit.sound);
    if (arrival.discovered) {
      updateSecrets();
      announce(
        `Secret ${pad(rest.found.size)} / ${pad(rest.spots.length)} · ${rest.visit.name}`,
      );
    }
    ...
    if ($("secret-line").hidden === resting) {
      $("secret-line").hidden = !resting;
      if (resting) $("secret-line").textContent = rest.visit.line;
    }
    const cameraSight = stopSight || (resting ? rest.visit.sight : null);
    const cameraPanel = stops.viewing ? panel : $("nearby-card");
    const cameraInset = stopSight
      ? cameraPanel.getBoundingClientRect().right + 24
      : 0;
    graphics.render(
      car,
      isSuspended() && started ? 0 : dt,
      !paused && !recovery.crashed,
      reducedMotion.matches,
      {
        sight: cameraSight,
        orbit: Boolean(stops.viewing),
        paused: orbitPaused,
        panelWidth: cameraInset,
        checkpoint: stops.checkpoint,
        crashed: recovery.crashed,
        parked,
      },
    );
    audio.update(car, control.throttle, paused, parked);
    // Text and the minimap refresh less often; world labels below still follow every camera frame.
    uiElapsed += dt;
    if (uiElapsed >= 0.075) {
      uiElapsed = 0;
      updateTiming();
      ...
      $("surface").textContent = recovery.crashed
        ? "BOOM!"
        : car.inWater
          ? "SPLASH"
          : !car.grounded && car.airtime > 0.12
            ? "AIRBORNE"
            : car.wheelspin > 0.5
              ? "WHEELSPIN"
              : car.surface === "snow"
      ...
      $("stage-name").textContent =
        roadAt(car.x, car.z)?.name || "THE SCENIC ROUTE";
      $("drift-status").classList.toggle("visible", car.drift && !paused);
      $("map-car").setAttribute(
        "transform",
        `translate(${car.x.toFixed(2)} ${car.z.toFixed(2)}) rotate(${((car.heading * 180) / Math.PI).toFixed(1)}) scale(2.8)`,
      );
      $("altitude").textContent = `${Math.round(car.y)} m`;
      $("ascent-meter").style.setProperty(
        "--ascent",
        `${Math.max(0, Math.min(100, (car.y / WORLD.summit) * 100))}%`,
      );
      if (!paused) discover();
      const road = roadAt(car.x, car.z);
      // Keep water recovery away from crossings so a failed jump respawns on a safe stretch of road.
      if (
        !paused &&
        car.grounded &&
        !car.inWater &&
        road &&
        road.distance < road.width / 2 &&
        CROSSINGS.every(
          (crossing) => Math.hypot(car.x - crossing.x, car.z - crossing.z) > 58,
        )
      ) {
    ...
    const cardBounds = stops.viewing
      ? panel.getBoundingClientRect()
      : $("nearby-card").hidden
        ? null
        : $("nearby-card").getBoundingClientRect();
    const labelVisible = (projected) =>
      projected.visible &&
      projected.x > 90 &&
      projected.x < window.innerWidth - 90 &&
      projected.y > (smallScreen ? 96 : 112) &&
      projected.y < window.innerHeight - (smallScreen ? 88 : 132) &&
      !(
        cardBounds &&
        projected.x < cardBounds.right + 100 &&
        projected.y > cardBounds.top - 10 &&
        projected.y < cardBounds.bottom + 40
      );
    landmarks.forEach((sight, index) => {
      const projected = graphics.project(
        sight.x,
        sight.type === "tower" ? 19 : sight.type === "arcade" ? 13 : 11,
        sight.z,
      );
      labels[index].hidden = Boolean(stops.viewing) || !labelVisible(projected);
      if (!labels[index].hidden)
        labels[index].style.transform =
          `translate(${Math.round(projected.x)}px, ${Math.round(projected.y)}px) translate(-50%, -100%)`;
    });
    crossingLabels.forEach((label) => {
      const projected = graphics.project(label.x, label.y, label.z, true);
      label.element.hidden =
        Boolean(stops.viewing) ||
        !labelVisible(projected) ||
        Math.hypot(car.x - label.x, car.z - label.z) > 95;
      if (!label.element.hidden)
        label.element.style.transform = `translate(${Math.round(projected.x)}px, ${Math.round(projected.y)}px) translate(-50%, -100%)`;
    });
    syncRendering();
  }
