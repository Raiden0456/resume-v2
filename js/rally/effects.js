...
export function createCrashEffect(scene) {
  ...
  const core = new Mesh(
    geometry,
    new MeshBasicMaterial({ color: "#ee935a", transparent: true, depthWrite: false }),
  );
    ...
    explode(car, reducedMotion) {
      ...
      for (const particle of particles) {
        const angle = Math.random() * Math.PI * 2,
          speed = particle.smoke ? 4 + Math.random() * 5 : 10 + Math.random() * 7;
...
export function createSplashEffect(scene) {
  ...
  return {
    ...
    update(dt) {
      if (!group.visible) return;
      age += dt;
      const t = Math.min(age / 0.85, 1),
        motion = still ? 0.22 : age;
      material.opacity = (1 - t) * 0.85;
      droplets.forEach(({ mesh, angle, speed, lift }, i) => {
        mesh.position.set(
          Math.cos(angle) * (0.7 + motion * speed),
          Math.max(0, motion * lift - 8 * motion * motion),
          Math.sin(angle) * (0.7 + motion * speed),
        );
        mesh.scale.set(0.16 + (i % 3) * 0.035, 0.3 + (i % 3) * 0.07, 0.16);
      });
      rings.forEach((ring, i) => {
        ring.scale.setScalar(1.1 + motion * (5 + i * 2));
        ring.position.y = i * 0.035;
      });
      if (t === 1) group.visible = false;
    },
    clear() {
  ...
  return {
    ...
    update(dt) {
      ...
      for (let i = 0; i < count; i++) {
        const burst = Math.floor(i / 60),
          delay = burst * 0.65,
          life = age - delay;
        const t = still ? 0.4 : Math.max(0, life),
          visible = still ? age < 2 : life > 0 && life < 1.8;
        const phi = i * 2.39996,
          elevation = Math.acos(1 - 2 * (((i % 60) + 0.5) / 60));
        ...
        const color = palette[burst],
          fade = visible ? Math.max(0, 1 - t / 1.8) : 0;
