export class RallyInput {
  constructor(surface) {
    ...
    window.addEventListener("keydown", (event) => {
      ...
      if (
        !event.repeat &&
        !interactive &&
        ["KeyR", "KeyE", "KeyM", "Escape"].includes(event.code)
      ) {
    ...
    // Keyup/pointerup may happen outside the page; clear held controls to avoid a stuck throttle.
  ...
  sample() {
    ...
    if (zones.has("left") || zones.has("right"))
      this.value.steer = Number(zones.has("right")) - Number(zones.has("left"));
    if (zones.has("gas") || zones.has("brake"))
      this.value.throttle = Number(zones.has("gas")) - Number(zones.has("brake"));
    if (zones.has("handbrake")) this.value.handbrake = true;
    if (!this.enabled) {
      this.value.throttle = 0;
      this.value.steer = 0;
