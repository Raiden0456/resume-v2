...
export function buildScenery(batch, landmarks, isRoad, routes) {
  ...
  const obstacles = [];
  const nearPath = (x, z) =>
    COTTAGE.path.some(([px, pz], i) => {
      const [qx, qz] = COTTAGE.path[Math.min(i + 1, COTTAGE.path.length - 1)],
        dx = qx - px,
        dz = qz - pz;
      const t = Math.max(
        0,
        Math.min(1, ((x - px) * dx + (z - pz) * dz) / (dx * dx + dz * dz || 1)),
      );
      return Math.hypot(x - px - dx * t, z - pz - dz * t) < 4;
    });
  const farFromSights = (x, z) =>
    landmarks.every(
      (sight) =>
        Math.hypot(x - sight.x, z - sight.z) > sight.radius + 9 &&
        Math.hypot(x - sight.marker[0], z - sight.marker[1]) > 13,
    );
  for (let i = 0; i < 8000; i++) {
    const x = random() * (WORLD.width - 80) - (WORLD.width - 80) / 2,
      z = random() * (WORLD.depth - 80) - (WORLD.depth - 80) / 2;
    const water = riverAt(x, z);
    if ((water && water.distance < water.width + 4) || isRoad(x, z, 5) || !farFromSights(x, z))
      continue;
    if (
      nearPath(x, z) ||
      SECRETS.some((secret) => Math.hypot(x - secret.x, z - secret.z) < secret.radius + 4)
    )
      continue;
    if (CABLEWAY && CABLEWAY.pylons.some((pylon) => Math.hypot(x - pylon.x, z - pylon.z) < 5))
      continue;
    ...
    if (meadow && i % 3 !== 0) {
      ...
      for (let k = 0; k < 3; k++) {
        const bx = x + (random() - 0.5) * spread * 2,
          bz = z + (random() - 0.5) * spread * 2,
          bs = 0.6 + random() * 0.9;
        batch.add(
          "rock",
          k % 2 ? "#4f7047" : "#5e8452",
      ...
      batch.add(
        "rock",
        snowy ? "#b9c0c1" : elevation > 190 ? "#8a8e86" : "#616d68",
        x,
        size * 0.4,
        z,
        size * 1.3,
  ...
  for (const main of routes)
    for (let i = 15; i < main.samples.length - 2; i += 19) {
      const [x, z] = main.samples[i],
        next = main.samples[i + 1];
      const angle = Math.atan2(next[1] - z, next[0] - x);
      for (const side of [-1, 1]) {
        const shoulder = main.width / 2 + 1.6;
        const px = x - Math.sin(angle) * side * shoulder,
          pz = z + Math.cos(angle) * side * shoulder;
        if (!farFromSights(px, pz)) continue;
        if (CAMPS.some((camp) => Math.hypot(px - camp.x, pz - camp.z) < camp.radius + 3)) continue;
        if (
          CROSSINGS.some(
            (crossing) => Math.hypot(x - crossing.x, z - crossing.z) < crossing.gap + 22,
          )
        )
          continue;
        ...
        const outerX = x - Math.sin(angle) * side * 22,
          outerZ = z + Math.cos(angle) * side * 22;
  ...
  for (const side of [-1, 1]) {
    ...
    for (let i = 0; i < 3; i++)
      camp("cylinder", "#252b2c", x + side * 1.5, 0.22 + i * 0.42, z + 2, 0.7, 0.4, 0.7);
  }
  ...
  for (let i = 0; i < 4; i++)
    summit("rock", "#a5aa9d", -2, 0.5 + i * 0.65, 1, 1.3 - i * 0.22, 0.5, 1.1 - i * 0.19);
  // Finish gantry in the valley, after the oldest chapter.
  const finish = (shape, color, x, y, z, sx, sy, sz, kind = "solid") => {
    const sin = Math.sin(FINISH_LINE.heading),
      cos = Math.cos(FINISH_LINE.heading);
    batch.absolute(
      shape,
      color,
      FINISH_LINE.x + x * cos - z * sin,
      FINISH_LINE.elevation + y,
      FINISH_LINE.z + x * sin + z * cos,
      sx,
      sy,
      sz,
      0,
  ...
  for (let i = 0; i < 18; i++)
    finish("box", i % 2 ? "#303a3c" : "#e2dcc7", i - 8.5, 7.5, 0.17, 0.9, 1.1, 0.06);
  for (const side of [-1, 1]) {
...
export function obstacleIndex(obstacles, cell = 40) {
  // Static obstacles are indexed by centre; neighbouring cells cover objects overlapping an edge.
  const grid = new Map();
  ...
  return (x, z) => {
    const gx = Math.floor(x / cell),
      ...
      key = `${gx},${gz}`;
    // Return a cached candidate list while the car stays in this cell; callers must not mutate it.
    if (key === lastKey) return nearby;
    lastKey = key;
    nearby = [];
    for (let dx = -1; dx <= 1; dx++)
      for (let dz = -1; dz <= 1; dz++) nearby.push(...(grid.get(`${gx + dx},${gz + dz}`) || []));
    return nearby;
  };
}
