...
export function makeWater(scene, batch) {
  ...
  const update = (dt) => {
    ...
    pending %= 1 / 30;
    for (let i = 0; i < 180; i++) {
      const cursor = ((i * (RIVER.length - 2)) / 180 + flow) % (RIVER.length - 2),
        j = Math.floor(cursor),
        t = cursor - j;
      const a = RIVER[j],
        b = RIVER[j + 1],
      ...
      dummy.rotation.set(
        -Math.PI / 2 + Math.atan2(b[2] - a[2], length),
        Math.atan2(-dx, -dz),
        0,
        "YXZ",
      );
  ...
  for (const crossing of CROSSINGS) {
    ...
    for (const [from, to] of sections) {
      ...
      for (let i = 0; i <= count; i++) {
        ...
        rows.push(
          [-1, 1].map((side) => {
            const across = side * crossing.halfWidth,
              { x, z } = crossingPoint(crossing, along, across);
      ...
      const deck = new THREE.Mesh(
        stripGeometry(rows),
        timber ? timberMaterial : crossing.type === "bridge" ? deckMaterial : rampMaterial,
      );
      ...
      for (let i = 0; i <= count; i += 3) {
        const along = from + ((to - from) * i) / count,
          { x, z } = crossingPoint(crossing, along);
        const y = crossingDeckHeight(crossing, along);
        const ahead = crossingPoint(crossing, along + 0.4),
          behind = crossingPoint(crossing, along - 0.4);
        const heading = Math.atan2(ahead.x - behind.x, behind.z - ahead.z);
        const pitch = Math.atan2(
          crossingDeckHeight(crossing, along + 0.4) - crossingDeckHeight(crossing, along - 0.4),
          Math.hypot(ahead.x - behind.x, ahead.z - behind.z),
        );
        add(
          "box",
          timber ? "#5e4630" : "#3f5557",
          x,
          y - 0.28,
          z,
          crossing.halfWidth * 2 + 0.25,
          0.5,
          0.55,
          pitch,
          -heading,
      ...
      if (timber)
        for (let i = 0; i <= count; i++) {
          ...
          add(
            ...
            "#6a4d31",
            x,
            crossingDeckHeight(crossing, along) + 0.045,
            z,
            crossing.halfWidth * 2,
            0.03,
            0.1,
            0,
            -Math.atan2(ahead.x - behind.x, behind.z - ahead.z),
            0,
          );
        }
      for (const along of [from + 1.5, to - 1.5])
        for (const side of [-1, 1]) {
          const { x, z } = crossingPoint(crossing, along, side * (crossing.halfWidth - 0.8));
          const top = crossingDeckHeight(crossing, along),
            bottom = terrainHeight(x, z) - 0.5;
    ...
    if (crossing.type === "jump") {
      for (const along of [-crossing.gap - 13, -crossing.gap - 9, -crossing.gap - 5]) {
        for (const side of [-1, 1]) {
          const { x, z } = crossingPoint(crossing, along, side * 0.8),
            y = crossingDeckHeight(crossing, along, side * 0.8);
          add(
            "box",
            "#efcf86",
            x,
      ...
      for (const side of [-1, 1]) {
        const { x, z } = crossingPoint(crossing, -crossing.gap, side * (crossing.halfWidth + 0.65)),
          y = crossingDeckHeight(crossing, -crossing.gap);
