...
export function createAtmosphere(scene, batch, landmarks) {
  ...
  function at(camp, across, along, turn = 0) {
    return frame(
      camp.x + Math.cos(camp.heading) * across + Math.sin(camp.heading) * along,
      camp.z + Math.sin(camp.heading) * across - Math.cos(camp.heading) * along,
      camp.heading + turn,
      camp.elevation,
    );
  }
  function solid(f, radius, height) {
    ...
    if (
      landmarks.some(
        (s) => Math.hypot(f.x - s.marker[0], f.z - s.marker[1]) < radius + 9,
      )
    )
      return;
  ...
  function sign(f, text, x, y, z, width = 5.5, accent = CREAM) {
    ...
    sprite.position.copy(f.point(x, y, z));
  ...
  function planter(f, x, z, color = "#75866a") {
    ...
    for (const dx of [-0.4, 0.25])
      f.add("rock", "#b9b592", x + dx, 1.9, z, 0.18, 0.16, 0.18);
  }
  ...
  function picnic(f) {
    ...
    for (const x of [-1.3, 1.3])
      f.add("box", "#63726e", x, 0.6, 0, 0.16, 1.15, 1.4);
    for (const z of [-1.4, 1.4]) {
      f.add("box", TIMBER, 0, 0.68, z, 3.8, 0.16, 0.65);
      for (const x of [-1.3, 1.3])
        f.add("box", "#63726e", x, 0.3, z, 0.16, 0.6, 0.55);
    }
    for (const x of [-0.8, 0.7])
      f.add("cylinder", CREAM, x, 1.4, 0.3, 0.14, 0.25, 0.14);
  ...
  function flag(f, color, height = 5) {
    ...
    const mesh = new THREE.Mesh(
      new THREE.PlaneGeometry(2.4, 1.1, 8, 1),
    ...
    mesh.position.copy(f.point(1.2, height - 0.75, 0));
    ...
    animated.push({
      ...
      tick(time) {
        ...
        for (let i = 0; i < vertices.count; i++) {
          const reach = (vertices.getX(i) + 1.2) / 2.4;
          vertices.setZ(
            i,
            Math.sin(time * 1.6 + reach * 4 + f.x) * reach * 0.24,
          );
        }
  ...
  function steam(f, x, y, z) {
    ...
    group.position.copy(f.point(x, y, z));
    ...
    animated.push({
      ...
      tick(time, still) {
        puffs.forEach((puff, i) => {
          ...
          puff.scale.setScalar(0.25 + life * 0.65);
          puff.material.opacity = (1 - life) * 0.14;
        });
      },
    });
  }
  function rotor(f, x, y, z, color = "#9ab4aa", radius = 1.4) {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      blade.position.set(
        (Math.sin(blade.rotation.y) * radius) / 2,
        0,
        (Math.cos(blade.rotation.y) * radius) / 2,
      );
      blade.scale.set(0.28, 0.12, radius);
      group.add(blade);
    }
    scene.add(group);
    animated.push({
  ...
  function beacon(f, x, y, z, color) {
    ...
    mesh.position.copy(f.point(x, y, z));
    scene.add(mesh);
    ...
    animated.push({
      ...
      tick(time) {
        material.color
          .copy(base)
          .multiplyScalar(0.72 + Math.sin(time * 1.5 + x) * 0.22);
      },
    });
  }
  function lights(a, b) {
    ...
    scene.add(
      new THREE.Line(
        ...
        new THREE.LineBasicMaterial({ color: "#677469" }),
      ),
    );
    for (const p of [a, b])
      batch.absolute("cylinder", "#7c8a80", p.x, p.y - 3.5, p.z, 0.1, 7, 0.1);
  }
  function stall(f, title, accent) {
    ...
    for (const x of [-2.7, 2.7])
      f.add("box", "#a8b5a4", x, 2.5, 1.85, 0.14, 4.8, 0.14);
    for (let i = 0; i < 8; i++)
      f.add(
        "box",
        i % 2 ? CREAM : accent,
    ...
    for (const x of [-1.5, -0.8, 0.9, 1.5])
      f.add(
        "cylinder",
        x < 0 ? "#ddaa70" : CREAM,
        x,
  ...
  function rallyCar(f, color, stripe, number, type = "coupe") {
    ...
    for (const x of [-0.72, 0.72])
      f.add("box", stripe, x, 1.52, -length * 0.35, 0.38, 0.05, length * 0.28);
    f.add("box", stripe, 0, 2.54, 0.4, 0.5, 0.04, 2.05);
    for (const x of [-1.4, 1.4])
      for (const z of [-length * 0.31, length * 0.32]) {
        f.add(
          "cylinder",
          "#19242a",
          x,
    ...
    solid(f, 2.6, 2.7);
    parkedCars.push({ x: f.x, z: f.z, type, number });
  }
  function service(f, accent) {
    for (const x of [-4.5, 4.5])
      for (const z of [-3.5, 3.5])
        f.add("box", "#adbbad", x, 2.5, z, 0.15, 5, 0.15);
    ...
    for (const y of [0.5, 0.9, 1.3])
      f.add("box", CREAM, -3, y, -1.95, 1.3, 0.07, 0.04);
    tyres(f, 3, -2);
    tyres(f, 3, 0);
  ...
  flag(at(start, -28, -9), "#d8b07b");
  flag(at(start, 15, -27), "#98c379");
  ...
  picnic(at(finish, -14, 8, Math.PI / 2));
  picnic(at(finish, -14, 20, Math.PI / 2));
  service(at(finish, 19, -24, Math.PI), "#859c90");
  const liveries = [
    ["#436f9f", "#d8bd75", "05", "sedan"],
    ["#e0dfce", "#72958a", "11", "coupe"],
    ["#ba705e", "#f0dbbc", "23", "hatch"],
    ["#bba37b", "#545f68", "46", "coupe"],
  ];
  liveries.forEach(([color, stripe, number, type], i) => {
    ...
    for (const x of [-2.2, 2.2])
      f.add("box", "#bdbd9d", x, 0.065, 0, 0.11, 0.02, 7.3);
  ...
  for (const [x, z, color] of [
    ...
    flag(at(finish, x, z), color, 6);
  lights(
    at(finish, -17, -10).point(0, 7, 0),
    at(finish, -17, 27).point(0, 7, 0),
  );
  lights(at(finish, 32, -17).point(0, 7, 0), at(finish, 32, 24).point(0, 7, 0));

  for (const sight of landmarks) {
    const f = frame(sight.x, sight.z),
      a = f.add;
    ...
    if (sight.type === "bank") {
      ...
      for (const x of [3, 5]) {
        for (const z of [6.6, 8])
          a("torus", "#89998f", x, 0.7, z, 0.6, 0.6, 0.6, 0, Math.PI / 2);
      ...
      for (const x of [-3.6, 0.4]) {
        ...
        for (const dx of [-1.1, 1.1])
          a("box", "#6f7c78", x + dx, 0.6, 9.3, 0.55, 0.9, 0.55);
      }
      ...
      for (const x of [-7, -3.5, 0]) {
        ...
        for (const dx of [-0.8, 0, 0.8])
          a("box", "#91aab3", x + dx, 1.75, 7.8, 0.035, 0.035, 2.7, -0.32);
      }
      rotor(f, 0, 16.8, 0, "#dfe6e4", 1.7);
      beacon(f, 0, 18.9, 0, "#98c379");
      sign(f, "SIGNAL / ONLINE", 0, 3.5, 6.5, 4.6, sight.color);
    } else if (sight.type === "arcade") {
      for (let i = 0; i < 4; i++)
        beacon(f, (i - 1.5) * 2.6, 3.2, 5.1, i % 2 ? "#e5c07b" : "#98c379");
      ...
      for (const x of [-1.7, 1.7])
        terrace.add("box", "#8a93b0", x, 0.55, 0, 0.75, 1.1, 0.75);
      ...
      for (const x of [7.1, 8.9])
        for (const z of [4.9, 6.2])
          a("cylinder", DARK, x, 0.4, z, 0.4, 0.2, 0.4, 0, 0, Math.PI / 2);
      for (const x of [7.4, 8.6])
        a("box", "#778981", x, 1.8, 4.3, 0.13, 3.5, 0.13);
      for (const x of [7.4, 8.6])
        a("box", "#a5b5a9", x, 0.15, 3.6, 0.18, 0.18, 2);
      beacon(f, 8, 1.7, 5.5, "#e2b66e");
      sign(f, "DISPATCH / 24H", -2, 5.2, 8, 6, sight.color);
    }
  }
  ...
  return {
    ...
    update(dt, reducedMotion, car) {
      ...
      const timeChanged =
        (dt > 0 && !reducedMotion) || reducedMotion !== lastReducedMotion;
      for (const animation of animated) {
        ...
        if (animation.light && active !== wasActive)
          animation.light.intensity = active ? animation.intensity : 0;
        if (active && (timeChanged || !wasActive))
          animation.tick?.(reducedMotion ? 0 : elapsed, reducedMotion);
      }
