...
export function createCarState(pose = SPAWN, heightAt = driveHeightAt) {
  ...
  return {
    ...
    pitch: Math.atan(
      slope.x * Math.sin(pose.heading) - slope.z * Math.cos(pose.heading),
    ),
    roll: Math.atan(
      slope.x * Math.cos(pose.heading) + slope.z * Math.sin(pose.heading),
    ),
    vx: 0,
    vz: 0,
    yaw: 0,
...
function isBump(state, verticalMomentum, planarSpeed, supportAt) {
  for (let ahead = 1; ahead <= BUMP_REACH; ahead++) {
    const t = ahead / planarSpeed,
      arc = state.y + verticalMomentum * t - 0.5 * GRAVITY * t * t;
    const gap =
      arc -
      supportAt(state.x + state.vx * t, state.z + state.vz * t, arc + 0.7);
...
export function stepCar(state, input, dt, environment = {}) {
  ...
  const throttle = clamp(input.throttle || 0, -1, 1),
    steering = clamp(input.steer || 0, -1, 1);
  ...
  let forwardX = Math.sin(state.heading),
    forwardZ = -Math.cos(state.heading);
  let rightX = Math.cos(state.heading),
    rightZ = Math.sin(state.heading);
  ...
  if (state.grounded) {
    const slope = terrainGradient(state.x, state.z, (x, z) =>
      supportAt(x, z, state.y + 1.1),
    );
    const forwardGrade = slope.x * forwardX + slope.z * forwardZ;
    const lowGear = 1 + 0.3 * clamp(1 - Math.abs(forward) / 8, 0, 1);
    const acceleration =
      throttle < 0 && forward > 0.6
        ? 29
        : throttle > 0 && forward < -0.6
          ? 24
          : 14 * lowGear;
    const thrust =
      (throttle * acceleration * (snow ? 0.8 : 1)) /
      Math.hypot(1, forwardGrade);
    forward += thrust * dt;
    const downhillTop =
      TOP_SPEED + DOWNHILL_BONUS * clamp(-forwardGrade / 0.5, 0, 1);
    state.topSpeed =
      downhillTop >= state.topSpeed
        ? downhillTop
        : Math.max(downhillTop, state.topSpeed - 6 * dt);
    if (Math.abs(forward) > 0.8 || throttle) {
      ...
      const gravity =
        (HILL_GRAVITY * forwardGrade) / (1 + forwardGrade * forwardGrade);
      const uphill = forward * forwardGrade > 0;
      if (throttle > 0 && forwardGrade > 0 && forward < 2)
        state.wheelspin = clamp(1 + (gravity + 1 - thrust) / 4, 0, 1);
      ...
      sideways -=
        clamp(slope.x * rightX + slope.z * rightZ, -1, 1) *
        (handbrake || state.wheelspin > 0.5 ? 1.3 : 0.35) *
        dt;
    }
    const resistance = !throttle ? (handbrake ? 3.8 : 3.2) : 0.65;
    forward -=
      Math.sign(forward) * Math.min(Math.abs(forward), resistance * dt);
    // Grass retains almost all of the pace; crossing a verge never clamps speed.
    forward *= Math.exp(-(onRoad ? 0.38 : 0.46) * dt);
    const slidingBrake =
      throttle || Math.abs(state.steer) > 0.1 || Math.abs(sideways) > 2;
    ...
    const corner =
      clamp((speed - 4) / 9, 0, 1) *
      clamp((Math.abs(state.steer) - 0.04) / 0.5, 0, 1);
    ...
    const powerSlide =
      throttle > 0 && forward > 0
        ? clamp((Math.abs(sideways) / Math.max(speed, 1) - 0.1) / 0.45, 0, 1) *
          clamp((speed - 5) / 7, 0, 1)
        : 0;
    const slide = handbrake
      ? clamp((speed - 3) / 5, 0, 1)
      : Math.max(corner, powerSlide);
    const baseGrip = onRoad ? 5.1 : 4.5;
    const targetGrip =
      (baseGrip + ((handbrake ? 1.05 : 2.05) - baseGrip) * slide) *
      (snow ? 0.6 : 1);
    // Grip breaks quickly but returns progressively, including on cambered
    // turns and after releasing the handbrake.
    state.grip +=
      (targetGrip - state.grip) *
      (1 - Math.exp(-(targetGrip < state.grip ? 12 : 2.6) * dt));
    const lateralBeforeGrip = sideways;
    sideways *= Math.exp(-state.grip * dt);
    if (!throttle)
      sideways -=
        Math.sign(sideways) *
        Math.min(Math.abs(sideways), (speed > 4 ? 0.3 : 2.5) * dt);
    ...
    forward =
      (Math.sign(forward) || Math.sign(throttle) || 1) *
      Math.sqrt(
        forward * forward +
          (lateralBeforeGrip * lateralBeforeGrip - sideways * sideways) * 0.97,
      );
    const speedLimit = Math.min(
      1,
      state.topSpeed / (Math.hypot(forward, sideways) || 1),
    );
    forward *= speedLimit;
    sideways *= speedLimit;
    state.vx = forwardX * forward + rightX * sideways;
    state.vz = forwardZ * forward + rightZ * sideways;
    if (!throttle && Math.hypot(state.vx, state.vz) < 0.65) {
      state.vx = 0;
      state.vz = 0;
      state.yaw = 0;
    }
    ...
    state.vx *= Math.exp(-0.025 * dt);
    state.vz *= Math.exp(-0.025 * dt);
  ...
  for (const obstacle of environment.obstacles || []) {
    if (
      state.y >
      (obstacle.elevation ?? heightAt(obstacle.x, obstacle.z)) +
        (obstacle.height ?? (obstacle.type === "tower" ? 17 : 10))
    )
      continue;
    let dx = state.x - obstacle.x,
      dz = state.z - obstacle.z;
    const radius = obstacle.radius + 1.15,
    ...
    const length = Math.hypot(dx, dz),
      nx = dx / length,
      nz = dz / length;
    state.x = obstacle.x + nx * radius;
    state.z = obstacle.z + nz * radius;
    const impact = state.vx * nx + state.vz * nz;
    if (impact < 0) {
      state.vx -= impact * nx * 1.25;
      state.vz -= impact * nz * 1.25;
      state.yaw *= 0.45;
    }
  }
  for (const rail of environment.rails || []) {
    if (state.y > rail.y + 1.2 || state.y < rail.y - 3) continue;
    const ex = rail.bx - rail.ax,
      ez = rail.bz - rail.az;
    const t = clamp(
      ((state.x - rail.ax) * ex + (state.z - rail.az) * ez) /
        (ex * ex + ez * ez || 1),
      0,
      1,
    );
    let dx = state.x - rail.ax - ex * t,
      dz = state.z - rail.az - ez * t;
    ...
    const length = Math.hypot(dx, dz),
      nx = dx / length,
      nz = dz / length;
    state.x += nx * (clearance - distance);
    state.z += nz * (clearance - distance);
    const impact = state.vx * nx + state.vz * nz;
    if (impact < 0) {
      state.vx = (state.vx - impact * nx * 1.3) * 0.94;
      state.vz = (state.vz - impact * nz * 1.3) * 0.94;
      state.yaw *= 0.5;
      state.suspensionVelocity -= Math.min(-impact * 0.1, 1.5);
    }
  }
  if (Math.abs(state.x) > WORLD.limitX) {
    state.x = clamp(state.x, -WORLD.limitX, WORLD.limitX);
    state.vx *= -0.2;
  }
  if (Math.abs(state.z) > WORLD.limitZ) {
    state.z = clamp(state.z, -WORLD.limitZ, WORLD.limitZ);
    state.vz *= -0.2;
  }

  let ground = supportAt(
    state.x,
    state.z,
    state.y + Math.max(0.7, state.vy * dt + 0.35),
  );
  let distance = Math.hypot(state.x - oldX, state.z - oldZ);
  // A rock face is a collision, not an elevator to its top.
  if (
    ground > state.y + Math.max(0.4, distance * 1.5, state.vy * dt + 0.3) &&
    distance < 2
  ) {
    state.x = oldX;
    state.z = oldZ;
  ...
  if (wasGrounded && distance > 0.0001 && ground > oldGround) {
    ...
    const projection =
      speed > 0.001
        ? clamp(
            (speed + state.vy * grade) / (speed * (1 + grade * grade)),
            0,
            1,
          )
        : 1;
  ...
  const verticalMomentum = wasGrounded
    ? Math.min(state.vy, state.launchVy)
    : state.vy;
  ...
  const bump =
    ...
    moving &&
    freeY > ground &&
    isBump(state, verticalMomentum, planarSpeed, supportAt);
  const crestRelease =
    !bump &&
    moving &&
    state.vy - groundVelocity > GRAVITY * dt * 1.2 &&
    freeY > ground;
  if (
    wasGrounded &&
    !crestRelease &&
    (bump || (!moving && ground >= state.y - 0.3) || freeY <= ground + 0.006)
  ) {
    // Slow cars must keep ground velocity, or the next incline projection eats their climb.
    state.y = ground;
    state.vy = groundVelocity;
    state.grounded = true;
    ...
    state.launchVy = moving
      ? state.launchVy +
        (state.vy - state.launchVy) * (1 - Math.exp(-distance / WHEELBASE))
      : 0;
  } else {
    ...
    if (
      ...
      const speed = Math.hypot(state.vx, state.vz);
      const grade =
        speed > 0.001 && Math.abs(groundVelocity) < 40
          ? groundVelocity / speed
          : 0;
      const projection =
        speed > 0.001
          ? clamp(
  ...
  state.suspensionVelocity +=
    (-95 * state.suspension - 10 * state.suspensionVelocity) * dt;
  state.suspension = clamp(
    state.suspension + state.suspensionVelocity * dt,
    -0.22,
    0.18,
  );
  const water = environment.waterAt?.(state.x, state.z);
  state.inWater = Boolean(
    water && water.distance < water.width && state.y < water.y - 0.15,
  );
  if (state.inWater) {
    state.vx *= Math.exp(-2.5 * dt);
    state.vz *= Math.exp(-2.5 * dt);
  }

  forwardX = Math.sin(state.heading);
  forwardZ = -Math.cos(state.heading);
  rightX = Math.cos(state.heading);
  rightZ = Math.sin(state.heading);
  ...
  state.slip = Math.abs(sideways);
  state.drift =
    state.grounded && !state.inWater && state.speed > 4 && state.slip > 2.2;
  const slope = terrainGradient(state.x, state.z, (x, z) =>
    supportAt(x, z, state.y + 1.2),
  );
  const targetPitch = state.grounded
    ? Math.atan(slope.x * forwardX + slope.z * forwardZ)
    : clamp(Math.atan2(state.vy, Math.max(8, state.speed)), -0.8, 0.7);
  const targetRoll = state.grounded
    ? Math.atan(slope.x * rightX + slope.z * rightZ)
    : state.steer * 0.08;
  state.pitch +=
    (targetPitch - state.pitch) *
    (1 - Math.exp(-(state.grounded ? 14 : 3.5) * dt));
