...
const curve = new CatmullRomCurve3(
  nodes.map(([x, z, y]) => new Vector3(x, y, z)),
  false,
  "centripetal",
);
let nodeIndex = 0;
...
export const SIGHTS = {
  ...
  "Supplier Success Accelerator": {
    id: "supplier",
    stage: 6,
    type: "accelerator",
    label: "THE BUSINESS ACCELERATOR",
    color: "#dfae5a",
    x: -89,
    z: -130,
    marker: [-71, -146],
  ...
  FaceStellar: {
    ...
    altitude: 38,
    summary:
      "A CRM system for a beauty salon, built with Node.js, Express, and React to support customer service.",
  },
  "Highway Logistic Group": {
    id: "highway",
    stage: 0,
    type: "depot",
    label: "THE VALLEY DEPOT",
    color: "#c9926b",
    x: 170,
    z: 322,
    marker: [150, 337],
...
function baseHeight(x, z) {
  ...
  const crags =
    Math.sin(x * 0.044 + z * 0.019) * Math.sin(z * 0.057) * 12 +
    Math.abs(Math.sin(x * 0.035 - z * 0.031)) * 10 +
    Math.sin(x * 0.14 + z * 0.087) * 2;
  height += crags * smoothstep(8, 100, height);
  const ravine =
    Math.exp(-(((x + 25 + z * 0.24) / 24) ** 2)) * Math.exp(-(((z - 5) / 90) ** 4)) * 65;
...
for (const route of ROUTES)
  for (let i = 0; i < route.samples.length - 1; i++) {
    ...
    const segment = {
      ...
      width: route.width,
      name: route.name,
      progress: courseDistance,
      length,
    };
    courseSegments.push(segment);
    courseDistance += length;
    for (
      let gx = Math.floor((Math.min(a[0], b[0]) - reach) / CELL);
      gx <= Math.floor((Math.max(a[0], b[0]) + reach) / CELL);
      gx++
    ) {
      for (
        let gz = Math.floor((Math.min(a[1], b[1]) - reach) / CELL);
        gz <= Math.floor((Math.max(a[1], b[1]) + reach) / CELL);
        gz++
      ) {
        const key = `${gx},${gz}`;
  ...
  for (const segment of roadGrid.get(`${Math.floor(x / CELL)},${Math.floor(z / CELL)}`) || []) {
    ...
    if (distance < best) {
      ...
      nearest = {
        ...
        width: segment.width,
        name: segment.name,
        progress: segment.progress + segment.length * t,
      };
    }
  }
  return nearest;
...
export const FINISH_LINE = {
  ...
  z: 390,
  elevation: 0,
  heading: roadAt(230, 390).heading,
  halfWidth: 9,
};
export const CAMPS = [
...
const PADS = Object.values(SIGHTS).map((sight) => ({
  x: (sight.x + sight.marker[0]) / 2,
  z: (sight.z + sight.marker[1]) / 2,
  y: sight.altitude,
}));
PADS.push(
  ...CAMPS.map((camp) => ({
...
export function landHeight(x, z) {
  ...
  if (road)
    height +=
      (road.y - height) * (1 - smoothstep(road.width / 2 + 3, road.width / 2 + 27, road.distance));
  for (const pad of PADS) {
    const distance = Math.hypot(x - pad.x, z - pad.z);
    if (distance < (pad.outer || 34))
      height += (pad.y - height) * (1 - smoothstep(pad.radius || 22, pad.outer || 34, distance));
  }
  return height;
}
...
const riverCurve = new CatmullRomCurve3(
  riverNodes.map(([x, z, y]) => new Vector3(x, y, z)),
  false,
  "centripetal",
);
let lastWaterHeight = Infinity;
export const RIVER = riverCurve.getPoints(1500).map((point, index) => {
  // Spline interpolation can overshoot vertically; water must never flow uphill along the route.
  lastWaterHeight = Math.min(lastWaterHeight, point.y);
  // River samples use [x, z, elevation, half-width], not Three.js's [x, y, z] order.
...
// secrets.js fills these registries during module initialization; height queries then include them.
...
for (const route of ROUTES)
  for (let i = 1; i < route.samples.length; i++) {
    const a = route.samples[i - 1],
      ...
      dx = b[0] - a[0],
      dz = b[1] - a[1];
    for (const { a: c, b: d, index } of riverGrid.get(
      ...
      // This is the sine of the crossing angle: oblique crossings need a longer span over the water.
      ...
      CROSSINGS.push({
        ...
        z,
        y: a[2] + (b[2] - a[2]) * t,
        style: bridgeIndex === 1 ? "timber" : "rail",
        heading: Math.atan2(dx, -dz),
        grade: (b[2] - a[2]) / Math.hypot(dx, dz),
        progress: roadAt(x, z).progress,
        halfWidth: route.width / 2 + 0.6,
        gap: c[3] / Math.max(0.3, angle) + (type === "jump" ? 1 : 2.2),
        rampLength: 17,
        rise: type === "jump" ? 6 : 0,
        riverIndex: index,
        label:
          type === "jump"
            ? route.name === "KNIFE EDGE"
              ? "RIDGE JUMP"
              : "VALLEY JUMP"
            : `${["SLATE", "FOREST", "CREEK"][bridgeIndex]} BRIDGE`,
      });
    }
  }
...
export function crossingCoordinates(crossing, x, z) {
  if (crossing.type === "bridge") {
    // Bridges follow the curved road; ramps use a straight takeoff/landing axis below.
    const road = roadAt(x, z);
    if (road)
      return {
        along: road.progress - crossing.progress,
        across: (x - road.x) * Math.cos(road.heading) + (z - road.z) * Math.sin(road.heading),
      };
  }
  const dx = x - crossing.x,
    dz = z - crossing.z;
  return {
    along: dx * Math.sin(crossing.heading) - dz * Math.cos(crossing.heading),
    across: dx * Math.cos(crossing.heading) + dz * Math.sin(crossing.heading),
  };
}
export function crossingPoint(crossing, along, across = 0) {
  if (crossing.type === "bridge") {
    ...
    let low = 0,
      high = courseSegments.length - 1;
    while (low < high) {
      const middle = Math.ceil((low + high) / 2);
      if (courseSegments[middle].progress <= progress) low = middle;
      else high = middle - 1;
    }
    const { a, b, progress: start, length } = courseSegments[low];
    const t = Math.max(0, Math.min(1, (progress - start) / length)),
      dx = b[0] - a[0],
      dz = b[1] - a[1];
    return { x: a[0] + dx * t - (dz / length) * across, z: a[1] + dz * t + (dx / length) * across };
  }
  return {
    x: crossing.x + Math.sin(crossing.heading) * along + Math.cos(crossing.heading) * across,
...
export function crossingDeckHeight(crossing, along, across = 0) {
  ...
  if (crossing.type === "bridge") {
    return (
      original +
      0.14 * smoothstep(0, 4, Math.min(along - crossing.deckStart, crossing.deckEnd - along))
    );
  }
  if (along <= -crossing.gap) {
    const t = Math.max(
      0,
      Math.min(1, (along + crossing.gap + crossing.rampLength) / crossing.rampLength),
    );
...
export function terrainHeight(x, z) {
  ...
  if (river && river.distance < river.width + 19) {
    const bank =
      river.y - 1.8 + smoothstep(river.width * 0.55, river.width + 7, river.distance) * 6;
    height += (bank - height) * (1 - smoothstep(river.width + 1, river.width + 19, river.distance));
    const road = roadAt(x, z);
    if (road && road.distance < road.width / 2 + 5) {
      const approach =
        smoothstep(river.width + 0.8, river.width + 3.5, river.distance) *
        (1 - smoothstep(road.width / 2, road.width / 2 + 5, road.distance));
  ...
  for (const corridor of CORRIDORS) {
    ...
    const target =
      corridor.startY + (corridor.endY - corridor.startY) * smoothstep(0, 1, trail.fraction);
    height +=
      (target - height) *
      (1 - smoothstep(corridor.width, corridor.outer, trail.distance)) *
      dry *
      smoothstep(0, 0.08, trail.fraction);
  }
  for (const site of TERRACES) {
    const terrace = Math.hypot(x - site.x, z - site.z);
    if (terrace < site.outer)
      height += (site.y - height) * (1 - smoothstep(site.radius, site.outer, terrace)) * dry;
  }
  return height;
}
...
for (const crossing of CROSSINGS.filter((crossing) => crossing.type === "bridge")) {
  for (const side of [-1, 1]) {
    ...
    for (; extent < crossing.gap + 70; extent++) {
      const centre = crossingPoint(crossing, side * extent),
        roadHeight = landHeight(centre.x, centre.z);
...
export function crossingSections(crossing) {
  return crossing.type === "bridge"
    ? [[crossing.deckStart, crossing.deckEnd]]
    : [
        [-crossing.gap - crossing.rampLength, -crossing.gap],
        [crossing.gap, crossing.gap + crossing.rampLength],
      ];
}
...
for (const crossing of CROSSINGS.filter((crossing) => crossing.style === "rail")) {
  for (const side of [-1, 1]) {
    ...
    for (let along = crossing.deckStart + 2; along < crossing.deckEnd + 2; along += 2) {
      const end = Math.min(along, crossing.deckEnd),
        point = crossingPoint(crossing, end, across);
      RAILS.push({
        ax: previous.x,
        az: previous.z,
        bx: point.x,
        bz: point.z,
        y: crossingDeckHeight(crossing, end - 1, across),
      });
      previous = point;
    }
  }
...
export function driveHeightAt(x, z, ceiling = Infinity) {
  // Physics supplies the highest reachable surface so a car under a bridge cannot snap onto it.
  // The default ceiling is useful when placing scenery or spawning directly on a deck.
  const ground = terrainHeight(x, z);
  for (const crossing of CROSSINGS) {
    const { along, across } = crossingCoordinates(crossing, x, z);
    if (
      Math.abs(across) > crossing.halfWidth ||
      !crossingSections(crossing).some(([from, to]) => along >= from && along <= to)
    )
      continue;
...
export function terrainGradient(x, z, heightAt = terrainHeight) {
  const step = 0.7;
  return {
    x: (heightAt(x + step, z) - heightAt(x - step, z)) / (step * 2),
    z: (heightAt(x, z + step) - heightAt(x, z - step)) / (step * 2),
  };
}
...
export function createLandmarks(resume) {
    ...
    for (const project of job.projects || [job.project || { name: job.company }]) {
      ...
      landmarks.push({
        ...
        description:
          design.summary ||
          project.description ||
          subsections.flatMap((section) => section.items || [])[0] ||
          job.role,
  ...
  return landmarks
    ...
    .map((sight, index) => ({
      ...sight,
      index,
      elevation: terrainHeight(...sight.marker),
      arrival: arrivalAt(sight.marker),
      courseDistance: roadAt(...sight.marker)?.progress ?? 0,
    }));
}
...
export function saveVisits(storage, visits) {
  try {
    storage.setItem("resume-rally-visits-v1", JSON.stringify([...visits]));
  } catch {
