...
export function createCameraRig(camera, groundAt = terrainHeight) {
  ...
  return {
    ...
    snap(target) {
      focus.copy(target);
      offset.copy(DRIVE_OFFSET);
      span = height < 550 ? 34 : 51;
      inset = 0;
      mode = "drive";
      sight = null;
      angle = START_ANGLE;
    ...
    update(drivingFocus, dt, view = {}, reducedMotion = false) {
      ...
      if (sight) {
        ...
        targetFocus.set(
          sight.x,
          sight.elevation + (sight.type === "tower" ? 8 : cottage ? 2 : 4),
          sight.z,
        );
        if (mode === "preview") {
        ...
        targetSpan = cottage
          ...
          : Math.max(
              minimumForBuilding,
              mode === "orbit" ? (height < 550 ? 18 : 21) : height < 550 ? 23 : 29,
            );
      }
      const blend = reducedMotion ? 1 : 1 - Math.exp(-(mode === "drive" ? 4 : 3) * dt);
      focus.lerp(targetFocus, blend);
      offset.lerp(desired, blend);
      span += (targetSpan - span) * blend;
      inset += (targetInset - inset) * blend;
      camera.position.copy(focus).add(offset);
      camera.lookAt(focus);
      project();
