...
function makeCar(scene) {
    ...
    if (!materials.has(key))
      materials.set(
        ...
        glow
          ...
          : new THREE.MeshStandardMaterial({ color, roughness: 0.63, metalness: 0.15 }),
      );
  ...
  for (const x of [-1, 1])
    for (const z of [-1.5, 1.5]) box(PALETTE.cream, x, 0.9, z, 0.42, 0.55, 1.14);
  ...
  const lights = [];
  const lit = (mesh, color, off) => {
    lights.push({ mesh, on: material(color, true), off: material(off) });
    return mesh;
  };
  ...
  nc.fillStyle = "#e2dfcd";
  nc.fillRect(0, 0, 64, 64);
  nc.fillStyle = "#303840";
  nc.font = "bold 42px monospace";
  nc.textAlign = "center";
  nc.fillText("04", 32, 48);
  const number = new THREE.Mesh(
    new THREE.PlaneGeometry(0.78, 0.8),
    new THREE.MeshBasicMaterial({ map: new THREE.CanvasTexture(numberCanvas) }),
  );
  number.rotation.x = -Math.PI / 2;
  number.position.set(0.46, 2.016, 0.2);
  body.add(number);
  const wheels = [];
  for (const x of [-1.12, 1.12])
    for (const z of [-1.43, 1.44]) {
      const steering = new THREE.Group();
      steering.position.set(x, 0.57, z);
      group.add(steering);
      const wheel = new THREE.Group();
      steering.add(wheel);
      const tyre = new THREE.Mesh(
        new THREE.CylinderGeometry(0.51, 0.51, 0.38, 12),
        material("#1b2025"),
      );
      ...
      const hub = new THREE.Mesh(
        new THREE.CylinderGeometry(0.31, 0.31, 0.4, 8),
        material("#c4c6b9"),
      );
  ...
  for (const x of [-0.8, 0.8]) {
    ...
    beam.position.set(x, 0.07, -9.8);
    group.add(beam);
    const light = new THREE.SpotLight("#ffedb5", 18, 22, 0.38, 0.75, 1.1);
    const position = new THREE.Vector3(x, 1.2, -2),
      target = new THREE.Vector3(x, 0, -13);
  ...
  for (const parent of [body, group]) {
    const parts = parent.children.filter(
      (child) => child.isMesh && child.geometry === boxGeometry && !lamps.has(child),
    );
  ...
  const sc = shadowCanvas.getContext("2d"),
    shade = sc.createRadialGradient(32, 32, 6, 32, 32, 31);
  shade.addColorStop(0, "#000000b0");
...
function makeTrails(scene) {
  ...
  return (car, dt, active) => {
    ...
    // The lower bound spaces tyre marks; the upper bound prevents trails across teleports.
    if (
      active &&
      car.grounded &&
      !car.inWater &&
      ...
      if (car.drift || car.slip > 1.3) {
        for (const side of [-1, 1]) {
          const x = car.x - sin * 1.4 + cos * side,
            z = car.z + cos * 1.4 + sin * side;
          const direction = Math.atan2(car.vx, car.vz),
            slope = terrainGradient(x, z, driveHeightAt);
          dummy.position.set(x, driveHeightAt(x, z) + 0.13, z);
          dummy.rotation.set(
            Math.atan(-slope.x * Math.sin(direction) - slope.z * Math.cos(direction)),
            direction,
            Math.atan(slope.x * Math.cos(direction) - slope.z * Math.sin(direction)),
            "YXZ",
          );
          dummy.scale.set(1, 1, Math.min(distance + 0.2, 1.5));
          dummy.updateMatrix();
      ...
      if (car.speed > 6) {
        ...
        particles[i] = {
          life: 1,
          vx: -car.vx * 0.08 + (Math.random() - 0.5) * 2,
          vz: -car.vz * 0.08 + (Math.random() - 0.5) * 2,
        };
    ...
    if (
      ...
      !car.inWater &&
      car.wheelspin > 0 &&
      Math.random() < car.wheelspin * dt * 40
    ) {
      const sin = Math.sin(car.heading),
        cos = Math.cos(car.heading),
      ...
      particles[i] = {
        life: 1,
        vx: -sin * 4 + (Math.random() - 0.5) * 2,
        vz: cos * 4 + (Math.random() - 0.5) * 2,
      };
    ...
    particles.forEach((particle, i) => {
      ...
      colors[i * 3] = 0.43 * particle.life;
      colors[i * 3 + 1] = 0.4 * particle.life;
      colors[i * 3 + 2] = 0.33 * particle.life;
    });
...
export function createScene(canvas, landmarks, { quality = "balanced" } = {}) {
  ...
  function isRoad(x, z, padding = 0) {
    if (CAMPS.some((camp) => Math.hypot(x - camp.x, z - camp.z) < camp.radius + padding))
      return true;
    if (
      landmarks.some(
        (sight) => Math.hypot(x - sight.marker[0], z - sight.marker[1]) < 8.5 + padding,
      )
    )
      return true;
    return (roadGrid.get(`${Math.floor(x / 20)},${Math.floor(z / 20)}`) || []).some(
      ([px, pz, radius]) => (x - px) ** 2 + (z - pz) ** 2 < (radius + padding) ** 2,
    );
  }
  ...
  const splashEffect = createSplashEffect(scene),
    fireworks = createFireworks(scene);
  ...
  const labelHeights = new Map(
    landmarks.map((sight) => [`${sight.x},${sight.z}`, terrainHeight(sight.x, sight.z)]),
  );
  ...
  car.lights.forEach((light) => {
    materials.add(light.on);
    materials.add(light.off);
  });
  let width = 0,
    height = 0,
    elapsed = 0;
  const resize = () => {
    ...
    const pixelRatio = Math.min(
      ...
      profile.pixelRatio,
      Math.sqrt(profile.maxPixels / (width * height)),
    );
  ...
  return {
    ...
    async prepare() {
      ...
      // Warm both shadow settings and parked lamps; changing either selects a different shader.
      try {
        for (const shadows of [true, false]) {
          renderer.shadowMap.enabled = shadows;
          for (const state of ["on", "off"]) {
            car.lights.forEach((light) => {
              light.mesh.material = light[state];
            });
    ...
    setQuality(next) {
      ...
      quality = next;
      profile = GRAPHICS_PROFILES[quality];
      renderer.shadowMap.enabled = profile.shadows;
      // Changing shadowMap.enabled alone does not invalidate Three.js's cached material program.
    ...
    render(state, dt, active, reducedMotion, view = {}) {
      ...
      // Physics heading and Three.js Y rotation have opposite signs for a car facing -Z.
      car.group.rotation.set(state.pitch, -state.heading, state.roll, "YXZ");
      car.body.rotation.z = reducedMotion
        ? 0
        : clamp(state.yaw * state.speed * 0.002, -0.065, 0.065);
      car.body.rotation.x = reducedMotion
        ? 0
        : Math.sin(elapsed * 26) * Math.min(state.speed / 2000, 0.015);
      ...
      car.beams.forEach((beam) => {
        beam.visible = headlightsOn;
      });
      // Headlights live outside the hideable car group to keep the renderer's light count stable.
      // Transform their local mounts and targets explicitly, including the car's pitch and roll.
      ...
      car.lights.forEach((light) => {
        light.mesh.material = view.parked ? light.off : light.on;
      });
      const ground = driveHeightAt(state.x, state.z, state.y + 0.7),
        clearance = Math.max(0, state.y - ground);
      const water = riverAt(state.x, state.z);
      car.shadow.position.set(
        state.x,
        Math.max(ground, water && water.distance < water.width ? water.y : ground) + 0.12,
        state.z,
      );
      ...
      car.wheels.forEach(({ wheel, steering, front }) => {
        wheel.rotation.x -=
          (state.speed *
            dt *
            (state.vx * Math.sin(state.heading) - state.vz * Math.cos(state.heading) < 0
              ? -1
              : 1)) /
          0.51;
      ...
      // Look along actual travel during a drift; use the nose at low speed where direction is noisy.
      target.set(
        (moving ? state.vx / state.speed : Math.sin(state.heading)) * lead,
        0,
        (moving ? state.vz / state.speed : -Math.cos(state.heading)) * lead,
      );
      ...
      rings.forEach((ring, index) => {
        ...
        ring.material.opacity = checkpoint
          ? 0.9
          : reducedMotion
            ? 0.6
            : 0.45 + Math.sin(elapsed * 1.7 + index) * 0.12;
    ...
    project(x, y, z, absolute = false) {
      ...
      return {
        ...
        visible:
          projection.z > -1 &&
          projection.z < 1 &&
          Math.abs(projection.x) < 1.1 &&
          Math.abs(projection.y) < 1.1,
      };
    },
    snapCamera(state) {
      lookAhead.set(Math.sin(state.heading) * 10, 0, -Math.cos(state.heading) * 10);
      target.set(state.x + lookAhead.x, state.y + 2, state.z + lookAhead.z);
      cameraRig.snap(target);
    },
  };
}
