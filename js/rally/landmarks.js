...
export function buildLandmark(batch, sight) {
  ...
  const c = sight.color,
    ...
    roof = "#3b454d",
    dark = DARK;
  const box = (color, x, y, z, w, h, d, kind = "solid") =>
    add("box", color, x, y, z, w, h, d, 0, 0, 0, kind);
  const [padWidth, padDepth] = {
    hub: [24, 18],
    showroom: [24, 18],
    web3: [22, 18],
  }[sight.type] || [18, 14];
  ...
  const windows = (width, height, z, columns, rows) => {
    for (let row = 0; row < rows; row++)
      for (let col = 0; col < columns; col++) {
        box(
          (col + row) % 4 ? "#aeaa7d" : "#3f5961",
          (col - (columns - 1) / 2) * width,
          1.8 + row * height,
          z,
          width * 0.55,
          height * 0.55,
  ...
  if (sight.type === "bank") {
    ...
    for (const x of [-4.8, -2.9, 2.9, 4.8])
      box("#f6f1ea", x, 2.7, 4.8, 0.65, 5, 0.7);
    box("#e4d6cf", 0, 0.5, 5.1, 13, 0.6, 2);
    box("#f2d9d2", 0, 7.2, 0, 4, 0.5, 4);
    for (let i = 0; i < 3; i++)
      add("cylinder", c, 0, 7.8 + i * 0.45, 0, 1.25, 0.3, 1.25);
    ...
    for (const x of [-3.9, -1.3, 1.3, 3.9])
      box("#c8d1c3", x, 7.2, 2.95, 0.14, 13.6, 0.12);
    ...
    for (const [x, h, color] of [
      ...
      for (let row = 0; row < Math.floor(h / 2.2); row++)
        box("#f5d9a6", x, 1.9 + row * 2.2, 4.86, 5.4, 0.45, 0.05, "light");
    }
    ...
    for (let i = 0; i < 6; i++) {
      const angle = (i / 6) * Math.PI * 2,
        nx = -8 + Math.cos(angle) * 1.9,
        nz = 8 + Math.sin(angle) * 1.9;
      add(
        "rock",
        i % 2 ? c : "#e2dfcd",
        nx,
        1.7 + Math.sin(i * 2.1) * 0.5,
        nz,
        0.42,
        0.42,
        0.42,
      ...
      add(
        "box",
        "#c8d1c3",
        -8 + Math.cos((angle + next) / 2) * 1.9,
        1.7 + (Math.sin(i * 2.1) + Math.sin((i + 1) * 2.1)) * 0.25,
        8 + Math.sin((angle + next) / 2) * 1.9,
        0.06,
        0.06,
        2,
        Math.atan2(Math.sin(i * 2.1) - Math.sin((i + 1) * 2.1), 4),
        -(angle + next) / 2 - Math.PI / 2,
        0,
      );
    }
    ...
    for (const x of [5, 7.2, 9.4]) {
      ...
      box(
        [c, "#f8efe0", "#243b5e"][Math.round((x - 5) / 2.2)],
        x + 0.75,
        7.1,
        8.6,
        1.5,
    ...
    for (const x of [-6.3, -3.4, -0.5, 2.4])
      box("#c2cebf", x, 3.1, 3.2, 0.14, 4.6, 0.12);
    for (let row = 0; row < 4; row++)
      box("#f5d9a6", 4.6, 1.8 + row * 2, 1.86, 3.2, 0.8, 0.08, "light");
    box(c, -2, 5.6, 3.4, 10.5, 0.25, 0.2, "light");
    box("#abb6a6", -2, 0.55, 5, 10.5, 0.35, 3.3);
    for (let i = 0; i < 3; i++)
      box(c, -4 + i * 1.55, 7.1 + i * 0.5, -1, 0.9, 1 + i, 0.8);
    ...
    for (const [x, z] of [
      ...
      box("#536f72", x, 0.7, z, 0.9, 0.9, 0.9);
      box(c, x, 1.3, z + 0.3, 0.9, 0.8, 0.15);
    }
    ...
    pixels.forEach((row, y) =>
      [...row].forEach((pixel, x) => {
        if (pixel === "1")
          box(c, (x - 3) * 0.65, 11 - y * 0.65, 0, 0.59, 0.59, 0.5, "light");
      }),
    );
    ...
    for (const side of [-1, 1])
      box("#4a5763", side * 10.55, 3.5, 0, 0.1, 6, 13, "glass");
    for (const z of [-7.3, 7.3]) {
      box("#d9cfb4", 0, 6.7, z, 22.5, 0.45, 0.5);
      box(gold, 0, 6.98, z, 22.5, 0.12, 0.5, "light");
    }
    for (const x of [-11, 11]) box("#d9cfb4", x, 6.7, 0, 0.5, 0.45, 15);
    for (let x = -9.6; x <= 9.7; x += 2.4)
      box("#c9b98f", x, 6.75, 0, 0.22, 0.28, 14.6);
    box("#c9b98f", 0, 6.8, 0, 22, 0.2, 0.22);
    for (const x of [-10.4, -5.2, 0, 5.2, 10.4])
      box("#c9b98f", x, 3.5, 6.5, 0.3, 6, 0.3);
    ...
    const luxury = (cx, cz, angle, body, trim) => {
      ...
      for (const x of [-0.95, 0.95])
        place("box", "#f1e6c2", x, 1.7, -3.18, 0.5, 0.2, 0.06, "light");
      for (const x of [-0.85, 0.85])
        place("box", "#c65d4c", x, 1.6, 3.19, 0.55, 0.16, 0.06, "light");
      place("box", gold, 0, 1.5, 0, 2.72, 0.06, 6, "light");
      for (const x of [-1.32, 1.32])
        for (const z of [-2, 2.05]) {
          place(
            "cylinder",
            "#101418",
            x,
    ...
    for (const z of [7.6, 9.6, 11.6, 13.4])
      for (const side of [-1, 1]) {
    ...
    for (const x of [-8, 8]) {
      add("cylinder", "#b7bfb2", x, 3.9, 9.4, 0.08, 7, 0.08);
      box(gold, x + 0.75, 6.9, 9.4, 1.5, 0.9, 0.06);
    }
    for (const x of [-7, 7]) {
      add("cylinder", "#4b4952", x, 0.8, -8.4, 0.18, 1.6, 0.18);
      add(
        "cone",
        "#f1e6c2",
        x,
    ...
    for (const y of [1.8, 4.4, 7])
      add("torus", c, 0, y, 0, 3.4, 3.4, 1, Math.PI / 2, 0, 0, "light");
    for (let i = 0; i < 4; i++)
      box(neon, 0, 2 + i * 1.4, 3.23, 0.28, 0.28, 0.08, "light");
    ...
    for (let i = 0; i < 7; i++) {
      ...
      const next = ((i + 1) / 7) * Math.PI * 2,
        ny = 4.6 + Math.sin((i + 1) * 1.9) * 1.1;
      const mx = Math.cos((angle + next) / 2) * r * 0.975,
        mz = Math.sin((angle + next) / 2) * r * 0.975;
      add(
        "box",
        neon,
        mx,
        ...
        0.07,
        0.07,
        Math.hypot(Math.cos(next) * r - x, ny - y, Math.sin(next) * r - z) -
          1.1,
        Math.atan2(
          y - ny,
          Math.hypot(Math.cos(next) * r - x, Math.sin(next) * r - z),
        ),
        -(angle + next) / 2 - Math.PI / 2,
        0,
        "light",
    ...
    for (const x of [-4.5, 0.3]) {
      ...
      for (let y = 0.9; y < 3.4; y += 0.55)
        box("#4d5b78", x, y, 3.68, 3.5, 0.06, 0.04);
    }
    for (let i = 0; i < 4; i++)
      box("#f0e2bf", -7.07, 4.3, -3.4 + i * 1.6, 0.06, 0.7, 1, "light");
    for (let i = 0; i < 2; i++) {
      box(i ? "#c48a5a" : "#33507a", 5.5, 1.6 + i * 2.5, -1, 3, 2.4, 8);
      for (let z = -4; z < 3; z += 0.7)
        box(i ? "#8e6340" : "#1d2b4a", 7.04, 1.6 + i * 2.5, z, 0.08, 2.2, 0.12);
    }
    box("#a7aaa0", -3, 1.6, 6, 6.2, 2.4, 2.6);
    box(c, 1.1, 1.4, 6, 2, 2, 2.6);
    box("#34525f", 1.3, 2, 7.32, 1.25, 0.7, 0.08);
    for (const x of [-5, -1, 1.4])
      for (const z of [4.6, 7.4])
        add("cylinder", "#20262a", x, 0.7, z, 0.55, 0.3, 0.55, Math.PI / 2);
  }
  ...
  for (let i = -5; i <= 5; i++)
    deck(
      ...
      0.055,
      i * 1.15,
      Math.sqrt(64 - (i * 1.15) ** 2) * 2,
      0.02,
      0.04,
    );
  const side = Math.sign(sight.x) || 1;
  for (const z of [-5, -2.5, 0, 2.5, 5])
    deck("box", "#a5b39c", side * 7, 0.85, z, 0.16, 1.7, 0.16);
