...
export class RallyRest {
  constructor(spots, storage) {
    this.spots = spots.map((spot) => ({
      ...spot,
      sight: { x: spot.x, z: spot.z, elevation: terrainHeight(spot.x, spot.z), type: "rest" },
    }));
    ...
    this.found = new Set();
    try {
      for (const id of JSON.parse(storage?.getItem(STORAGE_KEY) || "[]"))
        if (this.spots.some((spot) => spot.id === id)) this.found.add(id);
    } catch {}
  }

  update(car, dt, active) {
    const spot = active
      ? this.spots.find((spot) => Math.hypot(car.x - spot.x, car.z - spot.z) < spot.approach)
      : null;
