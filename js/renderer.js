...
(function () {
  ...
  function buildHero(data) {
    ...
    if (header) {
      ...
      if (info) {
        ...
        info.querySelector('.hero-location span').textContent = personal.location;
      }
    }
    ...
    if (contactRow) {
      ...
      var emailLink = el('a', {
        ...
        'aria-label': 'Email',
      }, icon('mail'), el('span', null, contact.email));
      ...
      var phoneLink = el('a', {
        ...
        className: 'contact-item',
        'aria-label': 'Phone',
      }, icon('phone'), el('span', null, contact.phone));
      ...
      if (contact.whatsapp) {
        var waLink = el('a', {
          ...
          'aria-label': 'WhatsApp',
        }, icon('message-circle'), el('span', null, contact.whatsapp.display));
    ...
    if (actionsRow) {
      ...
      if (data.pdf) {
        ...
        }, icon('download'), el('span', null, 'Download PDF'));
        actionsRow.appendChild(dlBtn);
      }

      var nav = el('nav', { className: 'social-links', 'aria-label': 'Social links' });

      var socialDefs = [
        { key: 'website',  iconName: 'globe',    label: 'Personal Website' },
  ...
  function buildTechStack(techStack) {
    ...
    Object.keys(techStack).forEach(function (category) {
      ...
      var card = el('div', { className: 'tech-category reveal' },
  ...
  function buildExperience(experience) {
    ...
    experience.forEach(function (job) {
      ...
      var companyNode;
      if (job.companyUrl) {
        companyNode = el('p', { className: 'job-company' },
          el('a', { href: job.companyUrl, target: '_blank', rel: 'noopener noreferrer' }, job.company),
          ' — ' + job.location
        );
      } else {
      ...
      meta.appendChild(
        el('span', { className: 'job-meta-item' }, icon('calendar'), ' ' + job.period)
      );
      meta.appendChild(
        el('span', { className: 'job-meta-item' }, icon('map-pin'), ' ' + job.location)
      );
      ...
      if (job.project) {
        ...
        var projHTML = '<strong>' + escapeHTML(job.project.name) + '</strong> — ' + escapeHTML(job.project.description);
        if (job.project.url) {
          projHTML += '<br><a href="' + escapeHTML(job.project.url) + '" target="_blank" rel="noopener noreferrer">'
      ...
      if (job.subsections && job.subsections.length) {
        job.subsections.forEach(function (sub) {
          ...
          sub.items.forEach(function (text) {
            var cls = 'job-detail-item' + (sub.style === 'results' ? ' result-item' : '');
            detailsWrap.appendChild(el('div', { className: cls }, text));
          });

          if (sub.title) {
            var subsection = el('div', { className: 'job-subsection' },
  ...
  function buildLanguages(languages) {
    ...
    languages.forEach(function (lang) {
      list.appendChild(
        el('div', { className: 'language-item' },
  ...
  function buildEducation(education) {
    ...
    education.forEach(function (edu) {
      var card = el('div', { className: 'education-card reveal' },
        el('h3', { className: 'education-degree' }, edu.degree),
        el('p',  { className: 'education-school' }, edu.institution),
        el('p',  { className: 'education-dates' }, icon('calendar'), ' ' + edu.period)
      );
      if (edu.description) {
        card.appendChild(el('p', { className: 'education-description' }, edu.description));
      }
  ...
  function buildCourses(courses) {
    ...
    courses.forEach(function (course) {
      var card = el('div', { className: 'course-card reveal' },
        el('h3', { className: 'course-name' }, course.name),
        el('p',  { className: 'course-institution' }, course.institution),
        el('p',  { className: 'course-dates' }, icon('calendar'), ' ' + course.period)
      );

      if (course.items && course.items.length) {
        var details = el('div', { className: 'course-description course-details' });
        var jobDetails = el('div', { className: 'job-details' });
        course.items.forEach(function (item) {
          jobDetails.appendChild(el('div', { className: 'job-detail-item' }, item));
        });
        details.appendChild(jobDetails);
        card.appendChild(details);
      } else if (course.description) {
        card.appendChild(el('p', { className: 'course-description' }, course.description));
      }
